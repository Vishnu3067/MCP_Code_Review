...
class LightweightRAGEngine:
    ...
    def retrieve(self, query: str, top_k: int = 6) -> List[str]:
        ...
        # Filter out very low similarity scores (lowered threshold for better recall)
        result_chunks = []
        for idx in top_indices:
            similarity_score = similarities[idx]
            if similarity_score > 0.001:  # Lower threshold to avoid filtering out relevant results
                result_chunks.append(self.chunks[idx])
                logger.info(f"Chunk {idx} similarity: {similarity_score:.4f}")
        
        logger.info(f"Retrieved {len(result_chunks)} chunks for query (top_k={top_k})")
        logger.info(f"Top similarity scores: {[f'{similarities[idx]:.4f}' for idx in top_indices[:3]]}")
    ...
    def generate_rag_prompt(self, query: str, top_k: int = 6) -> str:
        ...
        prompt = f"""You are a helpful assistant. Please provide a detailed response based on the context below. 
                ...
                Dont try to create any file or anything just reply back in the chat itself on above mentioned details.
...
def format_abap_artifacts_to_text(json_data: dict) -> str:
    """
    Convert OData JSON response to searchable text.
    Simple approach: turn each artifact dict into "Key: Value" text blocks.
    """
    # Step 1: Extract the list of artifacts from OData response
    artifacts = []
    if isinstance(json_data, dict):
        # Try common OData formats
        artifacts = json_data.get('value', [])  # OData V4 format
        if not artifacts:
            artifacts = json_data.get('results', [])  # Alternative format
    ...
    if not artifacts:
        ...
        logger.info(f"Response keys: {list(json_data.keys()) if isinstance(json_data, dict) else 'Not a dict'}")
        return "No data available"
    
    logger.info(f"Found {len(artifacts)} artifacts to process")
    
    # Step 3: Convert each artifact to text
    text_blocks = []
    for i, artifact in enumerate(artifacts):
        # Each artifact should be a dictionary
        if not isinstance(artifact, dict):
            continue
        
        # Build simple "Key: Value" format for each field
    ...
    logger.info(f"Successfully formatted {len(text_blocks)} artifacts")
    return "\n\n".join(text_blocks)
